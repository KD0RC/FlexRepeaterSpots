# FlexRepeaterSpots
FlexRepeaterSpots is a utility for Flex 6000, 8000 and Aurora series transceivers.  It places “permanent” spots on your SmartSDR screen (these spots do not time out like DX spots).  The spots indicate the frequency and name of FM repeaters.  A table of FM options is kept for each repeater spot such that clicking on a spot applies the appropriate options (mode, offset and direction, CTCSS tone, squelch) to the active slice (i.e. the slice that just landed on the spot).
